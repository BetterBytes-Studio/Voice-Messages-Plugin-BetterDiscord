...
module.exports = (() => {
  ...
  const { showToast, getData, saveData, findModuleByProps } = BdApi;
  ...
  class VoiceMessages {
      ...
      startRecording() {
          ...
          const settings = getData("VoiceMessages", "settings") || {};
          ...
          discordVoice.stopLocalAudioRecording(filePath => {
              ...
              require("fs").readFile(filePath, {}, (err, buf) => {
                  if (err) {
                      ...
                      showToast("❌ Failed to read recording file.", { type: "error" });
                      return;
                  }

                  const channelId = findModuleByProps("getLastSelectedChannelId").getLastSelectedChannelId();
                  const filenameFinal = useRandomFilename ? this.generateRandomFileName() : filename;

                  findModuleByProps("instantBatchUpload", "upload").instantBatchUpload({
                      channelId,
                      files: [
                          new File([
                  ...
                  showToast("🎙️ Recording uploaded successfully!", { type: "success" });
              });
          });
      }
      ...
      onStart() {
          ...
          showToast("VoiceMessages plugin started!", { type: "info" });
      }

      onStop() {
          document.removeEventListener("keydown", this.onKeydown.bind(this));
          showToast("VoiceMessages plugin stopped!", { type: "info" });
      }
